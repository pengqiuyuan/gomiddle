...
func GrayAccountHandler() {
	...
	http.HandleFunc("/kdsserver/server/getTotalByServerZoneIdAndGameId", TcpProtoIDKdsGrayGetTotalByServerZoneIdAndGameId)
}
...
func TcpProtoIDKdsGrayGetTotalByServerZoneIdAndGameId(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		serverZoneId := r.FormValue("serverZoneId")
		gameId := r.FormValue("gameId")
		...
		if exists {
			...
			connid, _ := gomiddle.ConnMa[serverId]
			conn.Send(connid, makeNoticeMsg(JsonStr,proto.TcpProtoIDKdsGrayGetTotalByServerZoneIdAndGameId))	

			select {
			case x := <-gomiddle.Channel_c:
				fmt.Println(serverId, "  存在,客户端有返回值  TcpProtoIDKdsGrayGetTotalByServerZoneIdAndGameId  ",proto.TcpProtoIDKdsGrayGetTotalByServerZoneIdAndGameId)
