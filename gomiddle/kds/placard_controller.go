...
func PlacardHandler() {
    ...
    http.HandleFunc("/kdsserver/placard/getPlacardById", GetPlacardById)
}
...
/**
 * 查询运营大区、游戏下所有的公告
 * 参数 localhost:8899/kdsserver/placard/getAllPlacards?serverZoneId=1&gameId=1&pageNumber=1&serverId=kds_server_1&pageSize=1
 */
func GetAllPlacards(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		if exists {
			...
			conn.Send(connid, makeNoticeMsg(JsonStr,proto.TcpProtoIDKdsGetAllPlacards))	
			
	
			select {
			case x := <-gomiddle.Channel_c:
				fmt.Println(serverId, "  存在,客户端有返回值  getAllPlacards  " , proto.TcpProtoIDKdsGetAllPlacards)
...
func SavePlacard(w http.ResponseWriter, r *http.Request) {
	AddOrUpdatePlacard(proto.TcpProtoIDKdsSavePlacard, w, r)
}

func UpdatePlacards(w http.ResponseWriter, r *http.Request) {
	AddOrUpdatePlacard(proto.TcpProtoIDKdsUpdatePlacards, w, r)
}

/**
 * 根据id删除公告
 * 参数 localhost:8899/kdsserver/placard/delPlacardById?id=1&serverZoneId=1&gameId=1&serverId=kds_server_1
 */
func DelPlacardById(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		if exists {
			fmt.Println(r.FormValue("serverId"), "  存在   ", conn)
			connid, _ := gomiddle.ConnMa[serverId]
			conn.Send(connid, makeNoticeMsg(JsonStr,proto.TcpProtoIDKdsDelPlacardById))	
			
			select {
			case x := <-gomiddle.Channel_c:
				fmt.Println(serverId, "  存在,客户端有返回值  delPlacardById  ",proto.TcpProtoIDKdsDelPlacardById)
