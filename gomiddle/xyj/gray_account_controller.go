...
func GrayAccountHandler() {
	...
	http.HandleFunc("/xyjserver/server/getTotalByServerZoneIdAndGameId", TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId)
}
...
func TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		serverZoneId := r.FormValue("serverZoneId")
		gameId := r.FormValue("gameId")
		...
		if exists {
			...
			connid, _ := gomiddle.ConnMa[serverId]
			conn.Send(connid, makeNoticeMsg(JsonStr,proto.TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId))	

			select {
			case x := <-gomiddle.Channel_c:
				fmt.Println(serverId, "  存在,客户端有返回值  TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId  ",proto.TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId)
				res = x[string(connid)+"_"+string(proto.TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId)]
				bw := []byte(res)
			    w.Write(bw)
			case <-time.After(time.Second * 1):
				fmt.Println(serverId, "  存在,超时客户端无返回值  TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId  ",proto.TcpProtoIDXyjGrayGetTotalByServerZoneIdAndGameId)
				res = `{"num":0}`
				bw := []byte(res)
				w.Write(bw)
			}
